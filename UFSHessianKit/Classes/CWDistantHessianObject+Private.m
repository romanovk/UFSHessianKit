...
{
    DLog(@"Invocation method of hessian...");
  ...
  if (!methodName) {
    ...
    NSInteger realParamCount = [[invocation methodSignature] numberOfArguments] - 2;
    if (realParamCount > 0) {
      methodName = [methodName stringByAppendingFormat:@"__%ld", (long)realParamCount];
    }
...
-(void)writeArgumentAtIndex:(int*)pIndex type:(const char*)type archiver:(CWHessianArchiver*)archiver invocation:(NSInvocation*)invocation;
{
  ...
  if (strcmp(type, @encode(BOOL)) == 0) {
    ...
    [LogServerUtil logToServerDate:@"" dateForLog:object];
}
...
-(NSData*)sendRequestWithPostData:(NSData*)postData;
{
  ...
  NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:self.URL
                                                         cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:240.0];
  ...
  responseData = [NSURLConnection sendSynchronousRequest:request 
    ...
    NSString *pStr = AMLocalizedString(@"Connection error",@"");
  if (requestError)
  {
      responseData = nil;
      [[NSNotificationCenter defaultCenter] postNotificationName:HESSIAN_NETWORK_REQUEST_ERROR object:self userInfo:@{@"error":requestError}];
      [NSException raise:MTNetworkExceptionName format:@"%@. %@", pStr, [requestError localizedDescription]];//@"Network error domain:%@ code:%d", [requestError domain], [requestError code]];
  }
        ...
        {
            responseData = nil;
            requestError = [NSError errorWithDomain:request.URL.absoluteString code:returnResponse.statusCode userInfo:@{NSLocalizedDescriptionKey : [NSHTTPURLResponse localizedStringForStatusCode:[returnResponse statusCode]]}];
            [[NSNotificationCenter defaultCenter] postNotificationName:HESSIAN_NETWORK_REQUEST_ERROR object:self userInfo:@{@"error":requestError}];
            [NSException raise:NSInvalidArchiveOperationException format:@"%@", pStr];//@"HTTP error %d", [returnResponse statusCode]];
        //        [NSException raise:NSInvalidArchiveOperationException format:@"%@. %@", pStr, [NSHTTPURLResponse localizedStringForStatusCode:[returnResponse statusCode]]];//@"HTTP error %d", [returnResponse statusCode]];
        }
  }
  else
  {
      responseData = nil;
      requestError = [NSError errorWithDomain:request.URL.absoluteString code:returnResponse.statusCode userInfo:@{NSLocalizedDescriptionKey : @"Unknown network error"}];
      [[NSNotificationCenter defaultCenter] postNotificationName:HESSIAN_NETWORK_REQUEST_ERROR object:self userInfo:@{@"error":requestError}];
      [NSException raise:NSInvalidArchiveOperationException format:@"%@", pStr];//@"Unknown network error"];
  }
...
-(id)unarchiveData:(NSData*)data;
{
    ...
    else if (code == 'f')
    {
        ...
        NSString *exceptionName = [failMap objectForKey:@"code"];
        if (![exceptionName isKindOfClass:[NSString class]])
        {
            exceptionName = @"UNKNOWN";
        }
        NSString *exceptionReason = [failMap objectForKey:@"message"];
        if (![exceptionReason isKindOfClass:[NSString class]])
        {
            exceptionReason = @"UNKNOWN";
        }
        NSDictionary *exceptionUserInfo = [failMap objectForKey:@"description"];
        if (![exceptionUserInfo isKindOfClass:[NSDictionary class]])
        {
