...
#ifndef NOT_OPTITMIZATION
	UInt8 *startPtr = CFDataGetMutableBytePtr((CFMutableDataRef)self.archiveData);
	UInt8 *endPtr = CFDataGetMutableBytePtr((CFMutableDataRef)self.archiveData) + [self.archiveData length];
#endif
	  
    for (int index = 0; index < len; index++) {
#ifdef NOT_OPTITMIZATION
	unichar ch = [self readChar];
#else
	unichar ch = *(startPtr+self.offset);
	self.offset++;
#endif
      if (ch < 0x80) {
...
#else
				if (startPtr+self.offset > endPtr) [NSException raise:NSInvalidArchiveOperationException format:@"bad utf-8 encoding"];
				unichar ch1 = *(startPtr+self.offset);
				self.offset++;
#endif
...
#else
				if (startPtr+self.offset > endPtr) [NSException raise:NSInvalidArchiveOperationException format:@"bad utf-8 encoding"];
				int ch1 = *(startPtr+self.offset);
				self.offset++;

				if (startPtr+self.offset > endPtr) [NSException raise:NSInvalidArchiveOperationException format:@"bad utf-8 encoding"];
				int ch2 = *(startPtr+self.offset);
				self.offset++;
#endif
...
#else
		CFStringAppendCharacters((CFMutableStringRef)string, &ch, 1);
#endif
...
-(NSException*)readFault;
{
    ...
    NSString *exceptionName = code;
    if (![exceptionName isKindOfClass:[NSString class]])
    {
        exceptionName = @"UNKNOWN";
    }
    NSString *exceptionReason = [faultMap objectForKey:@"message"];
    if (![exceptionReason isKindOfClass:[NSString class]])
    {
        exceptionReason = @"UNKNOWN";
    }
    NSDictionary *exceptionUserInfo = faultMap;
    if (![exceptionUserInfo isKindOfClass:[NSDictionary class]])
    {
        exceptionUserInfo = nil;
    }
    return [NSException exceptionWithName:exceptionName reason:exceptionReason userInfo:exceptionUserInfo];
}
...
-(id)readMapWithTypedObject:(id)typedObject;
{
  ...
  if (typedObject) {
    ...
    typedObject = [[typedObject initWithCoder:self] autorelease];
