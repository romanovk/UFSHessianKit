...
static BOOL CWAddProtocolImplementationsToClass(Class aClass, Protocol* aProtocol) {
  ...
  for (int index = 0; success && index < count; index++) {
    ...
    if (type == @encode(BOOL)[0]) {
      ...
      DLog(@"Unknown type %c for property %@ in protocol %@", type, propertyName, NSStringFromProtocol(aProtocol));
      success = NO;
    }
    if (success) {
...
{
  if (!object) {
    [_instanceVariables removeObjectForKey:CWPropertyNameFromSelector(_cmd)];
  } else {
  	object = [object copy];
    [_instanceVariables setObject:object forKey:CWPropertyNameFromSelector(_cmd)];	
    [object release];
